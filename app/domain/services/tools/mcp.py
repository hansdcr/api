...
4.另外有可能有一些环境变量是存储在我们整个系统中，在初始化MCP服务的时候，需要传递进来的
...
8.同时缓存ClientSession+Tool-Schema，一个是客户端会话，一个是工具参数说明;
...
class MCPClientManager:
    ...
    async def _connect_mcp_server(self,server_name:str,server_config:MCPServerConfig)->None:
        """根据传递的服务名字和服务配置连接到单个mcp服务器"""
        try:
            #1.获取mcp服务的传输协议
            transport = server_config.transport

            #2.根据不同传输协议调用不同的方法连接MCP服务器
            if transport == MCPTransport.STDIO:
                ...
                raise ValueError(f"MCP服务[{server_name}]使用了不支持的传输协议")
        except Exception as e:
            logger.error(f"连接mcp服务器[{server_name}]出错: {str(e)}")
            raise
    ...
    async def _connect_stdio_server(self,server_name:str,server_config:MCPServerConfig)->None:
        ...
        try:
            ...
            read_stream, write_stream = stdio_transport #获取读取流和写入流
    ...
    async def _connect_streamable_http_servers(self,server_name:str,server_config:MCPServerConfig)->None:
        """根据配置和服务名字+配置连接streamable-http服务"""
        # 1.提取streamable服务器的连接url并判断是否存在
        url = server_config.url
        if not url:
            raise ValueError("连接streamable-http-mcp服务器需要配置url")

        try:
            # 2.建立streamable-http连接
    ...
    async def get_all_tools(self) -> List[Dict[str,Any]]:
        """获取所有MCP工具列表，返回LLM可用的工具参数声明列表并处理MCP的名字"""
        #1.定义一个变量存储所有结果
        all_tools = []
        #2.循环遍历所有缓存工具
    ...
    async def invoke(self,tool_name:str, arguments: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not original_server_name or not original_tool_name:
                raise NotFoundError(msg=f"服务器解析MCP工具不存在: {tool_name}")
            #7.获取该工具所属会话
