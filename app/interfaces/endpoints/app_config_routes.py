...
@router.post(
    ...
    response_model=Response[AgentConfig],
    summary="更新Agent配置信息",
    description=" 更新Agent配置信息,包含最大迭代次数、最大重试次数、最大搜索结果数"
)
async def update_agent_config(
    new_app_config: AgentConfig,
    app_config_service: AppConfigService = Depends(get_app_config_service)
) -> Response[AgentConfig]:
    """ 更新Agent配置信息 """
    updated_agent_config = await app_config_service.update_agent_config(new_app_config)
    return Response.success(
        msg="更新agent配置成功",
...
@router.get(
    ...
    description="获取当前系统的MCP服务列表，包含MCP服务名字、工具列表、启用状态",
)
...
async def delete_mcp_server(
    ...
    """根据服务名字删除MCP服务器"""
    await app_config_service.delete_mcp_servers(server_name)
...
@router.get(
    ...
    summary="获取a2a服务器列表",
    description="获取Manus项目中的所有已配置的a2a服务列表",
)
