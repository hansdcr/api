...
class RedisStreamTask(Task):
    ...
    def __init__(self,task_runner: TaskRunner) -> None:
        ...
        self._input_stream_name = RedisStreamMessageQueue(input_stream_name)
        self._output_stream_name = RedisStreamMessageQueue(output_stream_name)
    ...
    async def _on_task_done(self) -> None:
        """任务结束时的回调函数"""
        # 1. 检测task runner是否存在，如果存在则调用task runner的回调函数
    ...
    async def _execute_task(self) -> None:
        ...
        try:
            ...
            logger.error(f"任务[{self._id}]执行异常: {self(e)}")
        finally:
