...
class SearchTool(BaseTool):
    ...
    @tool(
        name="search_web",
        description="全网搜索引擎工具，当需要获取实时信息(如突发新闻、天气)、补充内部知识库未涵盖的信息，或核查事实，会返回相关网页摘要和链接",
        parameters={
            "query": {
                "type": "string",
                "description":"针对搜索引擎优化的查询字符串。请提取问题中的核心实体和关键词(3-5个)，避免使用完整的自然语言问句(例如将'今天北京的天气怎么样?'改成'北京 天气')"
            },
            "date_range": {
                "type": "string",
