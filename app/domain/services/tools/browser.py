...
class BrowserTool(BaseTool):
    ...
    @tool(
        ...
        parameters={
            "url": {
                "type": "string",
                "description": "要访问的完整URL，必须包含协议前缀(例如https://)",
            }
        },
        required=["url"],
    )
    async def browser_navigate(self, url:str) -> ToolResult:
        """传递url地址，使用浏览器导航至对应页面"""
        return await self.browser.navigate(url)


    @tool(
        name="browser_restart",
        description="重启浏览器并导航至指定URL，当需要重制浏览器时使用",
        parameters={
            "url": {
                "type": "string",
                "description": "要访问的完整URL，必须包含协议前缀(例如https://)",
    ...
    async def browser_restart(self, url:str) -> ToolResult:
        """重启浏览器并导航至指定URL,传递url地址，使用浏览器导航至对应页面"""
    ...
    async def browser_click(
        ...
        """传递页面元素索引或页面xy坐标点击对应的元素"""
        return await self.browser.click(index,coordinate_x,coordinate_y)


    @tool(
        name="browser_input",
        description="覆盖浏览器当前页面可编辑区域的文本(input/textarea输入框)，在填充输入框时使用",
        parameters={
            "text": {
                "type": "string",
                "description": "要填入到输入框的完整文本内容",
            },
            "press_enter": {
                "type": "boolean",
    ...
    @tool(
        ...
        parameters={
            "coordinate_x": {
                "type": "number",
                "description": "目标光标的X坐标",
            },
            "coordinate_y": {
                "type": "number",
                "description": "目标光标的Y坐标",
            }
        },
    ...
    @tool(
        ...
        description="从当前浏览器页面的下拉列表元素中选择指定选项，用于选择下来菜单中的选项",
        parameters={
            "index": {
                "type": "integer",
                "description": "需要操作的下来列表元素的索引(序号)"
            },
            "option": {
                "type": "integer",
    ...
    async def browser_scroll_down(self,to_down:Optional[bool] = None) -> ToolResult:
        """向下滚动当前浏览器页面，支持滚动一页或滚动到底部"""
        return await self.browser.scroll_down(to_down)


    @tool(
        name="browser_console_exec",
        description="在浏览器控制台中执行JavaScript代码,当需要指定自定义脚本时使用",
        parameters={
            "javascript": {
                "type": "string",
